...
y_mat = eye(num_labels)(y,:);	% convert y into eye matrix
X = [ones(m, 1) X];				% add bias to X

% input layer
a1 = X;
z2 = a1*Theta1';		% theta*x
...
% the cost function
part1 =  (-y_mat).*log(a3);
part2 = (1 - y_mat).*log(1 - a3);

% regularization term
part3 = (0.5*lambda/m) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));

% cost
J = sum((1/m) * sum(part1 - part2)) + part3;
...
% for t = 1:m,
	...
	% % calculate error
	% del3 = a3 - y_mat;
	% del2 = (del3*Theta2(:,2:end)).*sigmoidGradient(z2);	% exclude bias unit from theta2
	% % update error
